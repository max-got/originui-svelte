...
<div>
	<Button
		variant="ghost"
		class="h-7 max-w-full whitespace-normal text-xs font-normal text-muted-foreground/80 hover:bg-transparent hover:text-foreground disabled:opacity-100"
		onclick={handleCopy}
	...
	>
		<div class="flex-[1]">
			{#if copied}
				Copied
			{:else}
				<span class="transition-colors">Copy class</span>
			{/if}
		</div>
